    ...
    return {
        init: function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response[0]?.data?.html) {
                        block.find('.hellocharly-loading').html(response[0].data.html);
                    } else {
                        block.find('.hellocharly-loading').html('<p>⚠️ Aucune donnée reçue.</p>');
                    }
                },
                error: function (error) {
                    console.error('❌ Erreur AJAX Hello Charly :', error);
                    block.find('.hellocharly-loading').html('<p>⚠️ Erreur de chargement des données Hello Charly.</p>');
                }
            });
            ...
            block.on('click', '.hellocharly-access-btn', function (e) {
                e.preventDefault();
                const button = $(this);
                block.find('.hellocharly-loading').show();
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        console.log('📦 Réponse SSO Hello Charly', response); // 👈 AJOUTE ÇA
                    ...
                    complete: function () {
                        block.find('.hellocharly-loading').hide();
